...
Field detection is **DONE** (`required_empty=0` on every job). Per-ATTEMPT submit rate with sparse production data is **~60-90%** (run-to-run variance); ALL remaining failures are in Greenhouse's post-submit **email-code step**, and the bounded 3× retry lifts CUMULATIVE success to ~95% across the day's 12 cron runs. To raise the per-attempt rate:
1. **Security-code field sometimes doesn't appear / email is slow.** Two sub-cases seen: (a) `submit_unconfirmed` with NO `code_not_received` → the code field didn't render within the 15s poll (SPA timing or the submit didn't take); (b) `code_not_received` (e.g. Intercom) → the "Security code…" email didn't reach the Resend inbound mailbox in ~32s. Ideas: longer/again-poll for the field, re-click submit once if neither confirmation nor code-field appears, and check `_poll_greenhouse_code()`'s sender/subject filter (`"greenhouse" in from`, `"security code" in subject`, `company[:6] in subject`) vs slow MX. **Validate by reproducing in-container** (`/tmp/prod_harness.py` pattern: sparse user_data + fresh jobs).
