...
- **Resume PDF: distinct templates + faster, font-correct render** (commit `211baa7`). Root cause of "all templates look the same / small / unprofessional / slow download": the worker ran as a non-root user with `HOME=/nonexistent` and no `XDG_CACHE_HOME`, so fontconfig couldn't cache and rescanned every render ("No writable cache directories" → slow); and only `fonts-liberation` was installed, so the templates' Calibri/Garamond/Helvetica all collapsed to one face. Fix: install Carlito (≈Calibri), EB Garamond, Caladea, DejaVu + `fc-cache` at build + writable `HOME/XDG_CACHE_HOME` (renders now ~180-375ms, no errors); retuned `_common.css` (bigger name, clearer hierarchy, italic company, font-independent round bullet). Also the picker **only applied a template if you clicked "Save" before "Download"** — now selecting a template auto-saves AND the Download link carries `?template=<id>` (honored by `/api/resumes/[id]/pdf`), so the PDF always matches the selection. Five templates now render visibly distinct (sans / centered serif / dense / blue-accent / green new-grad), all single-column ATS-safe. NOTE: auto-apply still uploads the reportlab text PDF (`careerops._render_resume_pdf`), NOT these templates — wiring the chosen template into the apply upload is a future nicety.
- **Credential-less LinkedIn → Greenhouse fallback** (commit `a46fbda`). LinkedIn Easy Apply needs the user's LinkedIn login; most users have none. A LinkedIn campaign with no stored creds now runs via the CAREEROPS (Greenhouse) engine (no credentials needed, replies still reach the inbox). LinkedIn campaigns WITH creds still use the LinkedIn path. So "auto-apply without credentials" works for everyone.
...
- **LinkedIn Easy Apply**: code path is complete (UI → encrypted creds → `worker/worker/autoapply/linkedin.py`, now with honest verification). Blockers: needs a user's LinkedIn password; **datacenter-IP login likely triggers LinkedIn security checkpoints**; account-ban risk. To validate, get a throwaway LinkedIn account and test login from the VPS IP. May need residential proxies. NOTE: as of `a46fbda`, credential-less LinkedIn campaigns auto-fall-back to the Greenhouse engine, so users without LinkedIn still get applications — LinkedIn-with-creds remains the only path that actually touches LinkedIn.
